...
- **Q6（程序性，非研究問題）watching（2026-08-01 run4 新增）** — Yang-Zhang/Garman-Klass
  交叉驗證連續三輪（run1~3）列入誘惑清單但連續三輪都不敢動手，原因是這牽涉「振幅怎麼算」
  本身，而 `program.md` §4／§3 紅線 5 明文把「度量」列為凍結網格的一部分——與 Q1'（換
  重抽單位）、Q1''（換樣本分層）不同，這兩者都沒動「振幅公式」本身。**懸而未決的是
  定性問題**：換一個更精確的波動率估計量重新檢驗同一個問題，算不算「改網格」需要走
  `program.md` §5 frontier 兩階段閘門？還是屬於「同一問題換更精確量尺」的穩健性覆核？
  **這個判斷本迴圈不自行下**，留待人類裁決；裁決前不應執行 YZ/GK 交叉驗證。
