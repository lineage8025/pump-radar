...
>
> 運行方式（**2026-08-05 改制**）：**每 2 小時一輪**，台北每奇數 UTC 小時的 :25
> 共 12 輪／天（**2026-08-06 ~ 08-08 為試跑期，由 workflow 的 `SCHEDULE_FROM` /
> `SCHEDULE_UNTIL` 自我啟停**，之後是否續跑由使用者決定），
...
>
> ⚠ **本階段目標擴大為「找出能真實交易的方向與策略」，因此新增一份位階更高的預登記文件：
> `docs/TRADEABILITY_PREREG.md`**（命題界定、假設預算與族系錯誤率、holdout 封存、升級判準）。
> **本檔與該檔衝突時，以該檔為準。** 一天 12 輪的強度會把 p-hacking 壓力放大一個量級，
> 該檔的 §3／§4 就是為此而設——不是形式，是這個階段唯一能讓結論可信的東西。
...
2. 讀 `docs/DETECTOR_PREREG.md`（偵測器預登記文件——參數與計分口徑的最高法律，凌駕本檔）、
   **`docs/TRADEABILITY_PREREG.md`（可交易性預登記——命題界定、假設預算、holdout 封存、
   升級判準；位階高於本檔，本階段每輪必讀）**、
...
| 車道 | 內容 | 產出 |
|---|---|---|
| `probe` | **主業**。執行 `docs/RND_BACKLOG.md` 已登記方向的探針（目前＝**方向二 Phase 0（Q10）**與**方向三 Phase A/B/C（Q11）**；方向一已於 2026-08-03 收攤），照各方向自己的凍結網格算，不自創網格 | 探針程式碼＋結果表＋研究日誌＋ledger 行 |
...
### 2.1 節流（排程運行專屬，最重要的一條）

**2026-08-05 起是每天 12 輪**，而 backlog 的實質工作量遠少於 12 輪。
**沒事做的那幾輪，正確行為是寫一行 ledger（status=`blocked`）後結束，不是找事做。**

⚠ **輪次密度提高後，本條的重要性也隨之提高，不是降低。** 12 輪／天意味著
「硬擠出一份日誌」的誘惑會出現 12 次；每一次屈服都在帳本裡累積一個沒有證據價值的
格位，而依 `docs/TRADEABILITY_PREREG.md` §3.2，**累計格位數只增不減、宣告門檻只緊不鬆**
——為了填滿輪次而多跑的每一格，都在讓真正的發現更難被宣告。**空轉的成本不是零，是負的。**

**優先撿 `docs/RND_BACKLOG.md` 方向三（Q11）的工作**：aggTrades 逐日串流處理是
確定性、耗時、不產生任何假設的基礎建設，天生適合填充高密度輪次，
且不會消耗 §3.2 的格位預算。沒事做時先問「方向三還有沒有月份沒處理完」。
...
4. **不做次根方向預測**。15m 動能方向已被 DirProbe 92k 樣本證偽（~46%）。
   任何形如「訊號後會漲還是會跌」的題目一律不受理。
   **精確界定見 `docs/TRADEABILITY_PREREG.md` §1（人類簽署，2026-08-05）**：
   受理振幅命題、路徑形狀命題、方向中性結構的淨報酬；
   **提出任何題目前必須先跑 §1.3 的符號翻轉檢定並把推導寫進日誌**——
   把報酬序列整體乘以 −1 後結論仍成立＝振幅／路徑命題（受理），
   結論消失或反轉＝方向命題（不受理，記入誘惑清單後放棄）。
   這是**硬性前置步驟，不是事後檢查**；界定是精確化不是放寬，受理範圍沒有變大。
...
（2026-08-02 run6 已用 Garman-Klass／Yang-Zhang 交叉驗證：24h 點估計三者同向、
GK 與 YZ 只差 0.0169pp，**削弱**漂移偏誤為主要驅動力的疑慮，但未完全排除。）

**已知限制②：對照組是固定時刻日格點，不是隨機時點**（2026-08-02 run5 發現，人類裁決
結案不修，見 backlog Q7）。上方去叢集規則實作為「索引 +96 根」的貪婪步進，使取樣點
退化成每標的每天一筆且全部落在同一 UTC 時刻；event 則分佈於全部 24 個 UTC 小時。
24h 檔的相位敏感度已量化為 0.024pp（可忽略，因 T=24h 涵蓋完整日內週期），
**但短 T（1h／4h）從未量化**。任何引用短 T 的 Q2 系列結論必須同時引述本限制。

### 4.1 穩健性覆核 vs 改網格（人類裁決，2026-08-02，回應 backlog Q6）

**判準是時序，不是動了哪個欄位**：一項檢驗若在**結果產生之前**就被書面指名為已知限制或
建議覆核項，那它是穩健性覆核，可直接執行；若是看過結果之後才想到的新切法，那是改網格，
必須走 §5 frontier 兩階段閘門。p-hacking 防的是「掃到顯著為止」，不是「照預先寫下的限制去查」。

**據此，Yang-Zhang / Garman-Klass 交叉驗證予以放行**——`RESEARCH_BBW_VOLATILITY.md`
§3.3 與 §5.4 在 Q1 跑出任何數字之前就點名了 Parkinson 漂移偏誤並建議交叉驗證，
上方「已知偏誤」段也早已載明。執行時受以下四條約束：

1. **只准這兩個具名估計量，各跑一次**。不得掃估計量家族；想用第三個（Rogers-Satchell、
   已實現波動率等）＝事後挑量尺，必須走 §5 閘門。
2. **主結果永遠是凍結網格那版**。YZ/GK 只進限制段作為交叉驗證，不得取代 Q1 的主表，
   更不得因為某個估計量的數字比較好看就改引用它。
3. **兩者不一致時，結論是「不穩健」**，照實寫進日誌與 ledger（status 至少 `inconclusive`），
   不得挑對敘事有利的那個。
4. **只換振幅公式**，分桶／T／窗口／標的／去叢集一律不動。

### 4.2 窗口延長協定（人類裁決，2026-08-02，回應 backlog Q2''）

延長分析窗口**允許**，但必須整段重跑並雙報，不得只補在缺樣本的那一格：

1. **全網格重跑**，不得只重算 n 不足的分桶——只補缺格＝挑對自己有利的子集。
2. **原窗口結果仍是主結果**（它才是預登記的）；延長窗口版本並列呈現，標明是事後延長。
3. **必須加做時期分層**：把延長後的樣本切成原窗口期與新增期兩段獨立重算。這不只是額外
   工作——它直接對上 run 1 最強反駁第 1／3 點（宏觀 regime 混淆）：若效應在兩個不重疊的
   時期各自成立，「少數 regime 事件的偽相關」這個反駁就被大幅削弱；若只在其中一段成立，
   那正是反駁成立的證據，照實記錄。
4. 延長後的窗口寫進 `backlog.md` 該題，**寫完才准跑**，之後不得再為改善結果而變更。
...
- **`summary` 必須以 `[cells=N]` 開頭**（2026-08-05 起，`docs/TRADEABILITY_PREREG.md` §3.1）：
  N ＝本輪實際計算的「分桶 × T × 參數」組合數，**包含跑出來覺得不好看、沒寫進日誌的那些**。
  沒跑任何格位的輪次（`blocked`／`error`／純資料處理）寫 `[cells=0]`。
  申報不實＝紅線違規；格位數與探針的迴圈結構事後對得起來。
...
## 10. 交易化階段的附加紀律（2026-08-05 起）

本階段目標擴大為「找出能真實交易的方向與策略」。完整規則在
`docs/TRADEABILITY_PREREG.md`（位階高於本檔），此處只列**最容易犯的五條**：

1. **提題前先跑符號翻轉檢定**（§1.3）。不對稱的止盈／止損掃描已用此檢定砍掉並記錄在案
   （§1.4），**不得換皮重提**——那條路是數學上不通，不是資料不夠。
2. **每輪申報格位數**，LEDGER 的 summary 以 `[cells=N]` 開頭（§3.1）。
   宣告門檻用 `α = 0.05 / 累計格位數` 的 Bonferroni 調整（§3.2），**未調整與調整後 CI 並報**。
3. **統計顯著不等於能賺**：效應量須 > **0.28%**（＝ 2 × 來回成本 0.14%）才准往下走（§3.3）。
   上界都打不過門檻就不必再做下去。
4. **holdout 封存**：一律只用 **2023-01-01 ~ 2025-06-30**（暖機 2022-12）。
   **禁止觸碰 2025-07-01 之後的任何資料**（§4）。workflow guard 會檢查 `experiments/` 的
   新增行是否出現指向封存段的抓取參數，但那只是嚇阻——真正的保證是你在日誌裡的窗口聲明。
5. **`supports` 不等於採納**（§5.1）。agent 最多做到「關 3 穩健性」，
   過關的正確行為是開 `Frontier Proposal:` issue 交人類裁決，
   **不得自行宣告策略可用，Discord 摘要禁止出現「可以交易」「找到 edge」這類措辭。**

**本階段的成功定義是「快速產生可信的否定結論」**，不是找到策略。
